...
import { Search, Loader2, X, CheckCircle, ChevronDown } from "lucide-react";
import { t } from "@/lib/i18n";
import { useSession } from "next-auth/react";
import { WEAPON_DISPLAY_NAMES } from "@/lib/weapon-categories";

const SUB_CATEGORIES: Record<string, number[]> = {
  knives: [500, 503, 505, 506, 507, 508, 509, 512, 514, 515, 516, 517, 518, 519, 520, 521, 522, 523, 525, 526],
};
...
const KNIFE_LABELS: Record<number, string> = {
  ...
  517: "Paracord Knife", 518: "Survival Knife", 519: "Ursus Knife",
...
export function WeaponBrowser() {
  ...
  const weaponTypes = useMemo(() => {
    ...
    skins.forEach((s) => {
      if (s.weapon_defindex && !map.has(s.weapon_defindex)) {
        const label = WEAPON_DISPLAY_NAMES[s.weapon_name] || s.weapon_name.replace("weapon_", "").replace(/_/g, " ");
  ...
  const handleEquip = async (paintId: number, defindex: number, seed: number, wear: number) => {
    if (!session) return;
    setEquipping(true);
    setEquipMsg("");
    try {
      const res = await fetch("/api/skins/equip", {
        ...
        body: JSON.stringify({
          type: "skin",
          data: { defindex, paintId, seed, wear },
        }),
      });
      ...
      if (result.success) {
        ...
        setEquipMsg("Hata: " + (result.message || "Bilinmeyen hata"));
      }
    } catch {
  ...
  const filteredBySub = useMemo(() => {
    ...
    return skins.filter((s) => s.weapon_defindex === subFilter);
  }, [skins, subFilter]);
  ...
  const currentSubOptions = activeCategory === "knives"
    ? SUB_CATEGORIES.knives.map((d) => ({ defindex: d, label: KNIFE_LABELS[d] || `Knife ${d}` }))
    : weaponTypes.map(([d, l]) => ({ defindex: d, label: l }));
            ...
            {filteredSkins.map((skin, index) => (
              ...
              >
                <SkinCard
                  ...
                  selected={selectedSkin?.paint_id === skin.paint_id && selectedSkin?.weapon_defindex === skin.weapon_defindex}
                  onSelect={() => setSelectedSkin(selectedSkin?.paint_id === skin.paint_id && selectedSkin?.weapon_defindex === skin.weapon_defindex ? null : skin)}
                  onEquip={handleEquip}
                />
              </motion.div>
            ))}
